...
use crate::{
    ...
    server::{read_req::ReadRequest, write_req::WriteRequest},
    Error, Result,
};
    ...
    H: Handler,
{
    ...
    fn handle_rrq(&self, peer: SocketAddr, req: Request) {
        ...
        let fut = async move {
            ...
            let mut rr = ReadRequest::init(&mut reader, size, peer, &req, config, local_ip).await?;
            rr.handle().await;
        ...
        self.ex
            .spawn(run_req(fut, peer, reqs_in_progress, local_ip))
            .detach();
    }

    fn handle_wrq(&self, peer: SocketAddr, req: Request) {
        ...
        let fut = async move {
            let mut writer = handler
                .lock()
                .await
                .write_req_open(&peer, req.filename.as_ref(), req.opts.transfer_size)
                .await
                .map_err(Error::Packet)?;

            let mut wr = WriteRequest::init(&mut writer, peer, &req, config, local_ip).await?;
            wr.handle().await;
...
async fn run_req(
    ...
    if let Err(e) = fut.await {
        ...
        if let Err(e) = send_error(e, peer, local_ip).await {
            trace!(
                "Failed to send error packet (peer: {}, error: {}",
                &peer,
                &e
            );
        }
