use crate::packet::{Packet, PACKET_DATA_HEADER_LEN};
    ...
    W: AsyncWrite + Send + Unpin,
{
    ...
    async fn try_handle(&mut self) -> Result<()> {
        ...
        loop {
            ...
            let data = self.recv_data(block).await?;
            // Write data to file
    ...
    async fn recv_data(&mut self, block: u16) -> Result<Bytes> {
        for _ in 0..=self.max_retries {
            match self.recv_data_block(block).await {
                Ok(data) => {
                    // Data received, send ACK
                    self.ack.clear();
                    Packet::Ack(block).encode(&mut self.ack);
                    self.socket.send_to(&self.ack, self.peer).await?;
                    return Ok(data);
                }
    ...
    async fn recv_data_block(&mut self, block: u16) -> io::Result<Bytes> {
        let socket = &mut self.socket;
        let peer = self.peer;

        self.buffer
            .resize(PACKET_DATA_HEADER_LEN + self.block_size, 0);
        ...
        io_timeout(self.timeout, async move {
            loop {
                ...
                if let Ok(Packet::Data(recv_block, _)) = Packet::decode(&buf[..len]) {
                    if recv_block == block {
                        buf.truncate(len);
...
fn build_oack_opts(config: &ServerConfig, req: &Request) -> Option<Opts> {
